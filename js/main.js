import { loadContent, getFallbackContent } from './contentLoader.js';
...
window.__valeTennisModuleLoaded = true;
...
const MATCH_VISUAL_CAMERAS = {
  broadcast: { label: 'TV Broadcast', desc: 'Câmera ampla com placar, rastro da bola e leitura de rally.' },
  coach: { label: 'Câmera do técnico', desc: 'Mostra zonas, tendência de rally e tomada de decisão.' },
  tactical: { label: 'Mapa tático', desc: 'Prioriza heatmap, direção do saque e impacto do plano.' },
  mobile: { label: 'Mobile limpo', desc: 'HUD maior, menos ruído visual e botões fáceis no celular.' }
};
const MATCH_VISUAL_OVERLAYS = {
  standard: { label: 'Padrão', desc: 'Placar, nomes, bola e replay curto.' },
  tactical: { label: 'Zonas táticas', desc: 'Áreas de ataque/defesa, saque e devolução.' },
  heatmap: { label: 'Heatmap', desc: 'Mapa vivo dos últimos pontos e pressão territorial.' },
  cinematic: { label: 'Cinemático', desc: 'Visual mais limpo para parecer transmissão.' }
};
...
function saveOwnerSetup() {
  ...
  closeOwnerSetup(); hideStandaloneStartupShield?.(); hideStartScreenV2?.('carreira criada'); switchTab('dashboard'); render(); hydrateAssetImages(); return true;
}
...
function ensureStartScreenV2System() {
  state.startScreenV2 ||= {
    score: 100,
    visits: 0,
    lastReason: null,
    lastAction: null,
    dismissedForBuild: null,
    auditLog: [],
    flags: {
      premiumLanding: true,
      careerCreator20: true,
      blockDashboardUntilValid: true,
      continueRequiresValidCareer: true,
      guidedRecovery: true
    }
  };
  const ss = state.startScreenV2;
  ss.auditLog ||= [];
  ss.visits ??= 0;
  ss.lastReason ??= null;
  ss.lastAction ??= null;
  ss.dismissedForBuild ??= null;
  ss.flags ||= { premiumLanding: true, careerCreator20: true, blockDashboardUntilValid: true, continueRequiresValidCareer: true, guidedRecovery: true };
  ss.flags.premiumLanding ??= true;
  ss.flags.careerCreator20 ??= true;
  ss.flags.blockDashboardUntilValid ??= true;
  ss.flags.continueRequiresValidCareer ??= true;
  ss.flags.guidedRecovery ??= true;
  return ss;
}
function startScreenV2Snapshot() {
  ...
  return {
    ...
    valid, issues,
    ownerName: owner.name || '', avatar: owner.avatar || '', country: owner.country || '',
    academyName: state?.academy?.name || '', city: profile.city || '',
    roster: state?.roster?.length || 0, ranking: state?.ranking?.length || 0, calendar: state?.calendar?.length || 0,
    money: Number(state?.academy?.money || 0), currentTab: state?.ui?.currentTab || 'dashboard',
    overlayVisible: !!document.getElementById('startScreenV2') && !document.getElementById('startScreenV2').classList.contains('hidden')
  };
}
function calculateStartScreenV2Score(snap = startScreenV2Snapshot()) {
  let score = 100;
  score -= Math.min(52, snap.issues.length * 7);
  if (!snap.valid && !snap.overlayVisible) score -= 22;
  if (!snap.valid && snap.currentTab === 'dashboard') score -= 18;
  if (snap.roster < 1) score -= 8;
  return clamp(Math.round(score), 0, 100);
}
function logStartScreenV2(title, result='OK', note='', snap = startScreenV2Snapshot()) {
  const ss = ensureStartScreenV2System();
  ss.score = calculateStartScreenV2Score(snap);
  ss.lastAction = title;
  ss.auditLog.unshift({ title, result, note, score: ss.score, at: new Date().toISOString(), build: BUILD_INFO.build, snapshot: snap });
  ss.auditLog = ss.auditLog.slice(0, 24);
  saveState(state);
  return ss.score;
}
function renderStartScreenV2Overlay() {
  const overlay = document.getElementById('startScreenV2');
  if (!overlay || !state) return;
  const ss = ensureStartScreenV2System();
  const snap = startScreenV2Snapshot();
  ss.score = calculateStartScreenV2Score(snap);
  const title = snap.valid ? 'Sua carreira está pronta' : 'Comece criando sua carreira';
  const subtitle = snap.valid
    ? `${snap.academyName} • ${snap.ownerName} • ${snap.city || 'cidade definida'}`
    : 'Escolha treinador, país, cidade, nome da academia e avatar antes de liberar o Dashboard.';
  const canContinue = snap.valid;
  const issues = snap.issues.slice(0, 6);
  overlay.innerHTML = `<div class="start-v2-bg"></div><div class="start-v2-shell glass"><div class="start-v2-copy"><p class="eyebrow">Entrada Premium 2.0 • ${BUILD_LABEL}</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p><div class="start-v2-badges"><span>Nova Carreira</span><span>Creator 2.0</span><span>Mobile-first</span><span>Anti-Dashboard vazio</span></div><div class="start-v2-actions"><button class="btn-primary start-v2-main" type="button" onclick="window.startNewCareerV2()">Nova Carreira</button><button class="btn-secondary" type="button" ${canContinue ? '' : 'disabled'} onclick="window.continueCareerV2()">Continuar Carreira</button><button class="btn-ghost" type="button" onclick="window.recoverCareerV2()">Resetar / Recuperar</button></div><small class="start-v2-note">Se o jogo abrir zerado, esta tela bloqueia a entrada e preserva backup antes de resetar.</small></div><div class="start-v2-status"><div class="release-score ${snap.valid ? 'ok':'pending'}"><span>Start</span><strong>${ss.score}</strong><small>${snap.valid ? 'OK':'Criar'}</small></div><div class="start-v2-card"><h3>Estado da carreira</h3><div class="start-v2-list"><div><span>Treinador</span><strong>${escapeHtml(snap.ownerName || 'pendente')}</strong></div><div><span>Academia</span><strong>${escapeHtml(snap.academyName || 'pendente')}</strong></div><div><span>Elenco</span><strong>${snap.roster} atletas</strong></div><div><span>Ranking</span><strong>${snap.ranking} registros</strong></div></div></div><div class="start-v2-card ${snap.valid ? 'ok':'warn'}"><h3>${snap.valid ? 'Pronto para jogar' : 'Faltando'}</h3>${snap.valid ? '<p>Continue para o Dashboard com a carreira validada.</p>' : `<ul>${issues.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul>`}</div></div></div>`;
}
function showStartScreenV2(reason='entrada premium', force=false) {
  const overlay = document.getElementById('startScreenV2');
  if (!overlay || !state) return false;
  const ss = ensureStartScreenV2System();
  const valid = careerIsPlayableAndConfigured(state);
  if (valid && !force && ss.dismissedForBuild === BUILD_INFO.build) {
    overlay.classList.add('hidden');
    document.body.classList.remove('start-screen-open');
    return false;
  }
  ss.visits = (ss.visits || 0) + 1;
  ss.lastReason = reason;
  state.ui ||= {};
  if (!valid) {
    state.ui.currentTab = 'startscreen';
    state.ui.lastStableTab = 'startscreen';
    if (document.getElementById('tab-startscreen')) switchTab('startscreen');
  }
  renderStartScreenV2Overlay();
  overlay.classList.remove('hidden');
  document.body.classList.add('start-screen-open');
  logStartScreenV2('Tela inicial premium aberta', valid ? 'READY' : 'LOCKED', reason);
  return true;
}
function hideStartScreenV2(reason='continuar') {
  const overlay = document.getElementById('startScreenV2');
  if (!overlay) return;
  overlay.classList.add('hidden');
  document.body.classList.remove('start-screen-open');
  const ss = ensureStartScreenV2System();
  ss.dismissedForBuild = BUILD_INFO.build;
  ss.lastReason = reason;
  logStartScreenV2('Tela inicial fechada', 'OK', reason);
}
function launchStartScreenV2(reason='boot', force=false) {
  ensurePlayableStart();
  render();
  return showStartScreenV2(reason, force || !careerIsPlayableAndConfigured(state));
}
function renderStartScreenV2() {
  const hub = document.getElementById('startScreenV2Hub');
  if (!hub || !state) return;
  const ss = ensureStartScreenV2System();
  const snap = startScreenV2Snapshot();
  ss.score = calculateStartScreenV2Score(snap);
  const checks = [
    ['Carreira válida', snap.valid, snap.valid ? 'Pode continuar para o Dashboard' : snap.issues.join(', ')],
    ['Dashboard bloqueado quando inválido', !(snap.currentTab === 'dashboard' && !snap.valid), `Tab atual: ${snap.currentTab}`],
    ['Tela inicial disponível', !!document.getElementById('startScreenV2'), 'Overlay premium instalado'],
    ['Creator 2.0 pronto', !!document.getElementById('ownerSetupModal') && !!document.getElementById('ownerAvatarChoices'), 'Formulário e avatar selector detectados'],
    ['Build atual', true, BUILD_LABEL]
  ];
  hub.innerHTML = `<section class="onboarding-hero glass-card-lite start-v2-hub"><div><p class="eyebrow">Full Start Screen Rebuild • ${BUILD_LABEL}</p><h2>Tela inicial premium e Career Creator 2.0</h2><p>Esta fase troca o início improvisado por uma entrada clara: Nova Carreira, Continuar e Recuperar. O Dashboard só abre quando a carreira estiver válida.</p></div><div class="release-score ${ss.score >= 85 ? 'ok':'pending'}"><span>Start 2.0</span><strong>${ss.score}</strong><small>${snap.valid ? 'OK':'Criar'}</small></div></section><section class="onboarding-actions"><button class="btn-primary" onclick="window.launchStartScreenV2('aba Entrada 2.0', true)">Abrir tela inicial</button><button class="btn-secondary" onclick="window.startNewCareerV2()">Nova carreira</button><button class="btn-secondary" onclick="window.continueCareerV2()">Continuar</button><button class="btn-ghost" onclick="window.recoverCareerV2()">Resetar / Recuperar</button><button class="btn-ghost" onclick="window.exportStartScreenV2Report()">Exportar relatório</button></section><section class="onboarding-check-grid">${checks.map(([label,ok,note])=>`<article class="release-check ${ok?'ok':'pending'}"><span>${ok?'✓':'!'}</span><div><strong>${escapeHtml(label)}</strong><small>${escapeHtml(note || '')}</small></div></article>`).join('')}</section><section class="release-grid"><article class="panel-card"><h4>Histórico da entrada</h4><div class="list-block">${(ss.auditLog||[]).slice(0,6).map(item=>`<div class="list-item"><div><strong>${escapeHtml(item.title)}</strong><div class="small">${escapeHtml(item.note || '')}</div></div><b>${escapeHtml(item.result || String(item.score))}</b></div>`).join('') || '<div class="list-item"><span>Nenhuma ação registrada.</span><strong>Pronto</strong></div>'}</div></article><article class="panel-card"><h4>Rota recomendada</h4><code>index.html?start=1#startscreen</code><p class="muted">Use essa rota se quiser abrir sempre pela nova tela inicial premium.</p></article></section>`;
  renderStartScreenV2Overlay();
}
window.launchStartScreenV2 = launchStartScreenV2;
window.hideStartScreenV2 = hideStartScreenV2;
window.startNewCareerV2 = () => {
  try {
    const previous = localStorage.getItem('vale_tennis_manager_save');
    if (previous) localStorage.setItem(`vale_tennis_start_v2_backup_${BUILD_INFO.build}_${Date.now()}`, previous);
  } catch (error) { console.warn('Backup Start 2.0 indisponível', error); }
  clearState();
  state = buildInitialState(content);
  migrateState();
  state.flags ||= {};
  state.flags.ownerSetupComplete = false;
  state.ui = { currentTab:'startscreen', lastStableTab:'startscreen' };
  ensureStartScreenV2System().lastAction = 'Nova Carreira';
  saveState(state);
  render();
  hideStartScreenV2('abrir Career Creator 2.0');
  openOwnerSetup(true);
  logStartScreenV2('Nova Carreira acionada', 'OPEN', 'Creator 2.0 aberto com base limpa e backup local.');
};
window.continueCareerV2 = () => {
  if (!careerIsPlayableAndConfigured(state)) {
    showSystemError('Ainda falta criar nome, avatar, país, cidade, academia ou base jogável. Use Nova Carreira ou Recuperar.');
    showStartScreenV2('continuar bloqueado', true);
    return false;
  }
  hideStartScreenV2('continuar carreira');
  switchTab('dashboard');
  render();
  return true;
};
window.recoverCareerV2 = () => {
  rebuildPlayableCareer('recuperação Start Screen 2.0');
  state.flags ||= {};
  state.flags.ownerSetupComplete = false;
  state.ui = { currentTab:'startscreen', lastStableTab:'startscreen' };
  ensureStartScreenV2System().lastAction = 'Recuperar';
  saveState(state);
  render();
  hideStartScreenV2('abrir recuperação no Career Creator 2.0');
  openOwnerSetup(true);
  logStartScreenV2('Recuperação acionada', 'REPAIR', 'Base jogável recriada e Creator 2.0 aberto.');
  return true;
};
window.exportStartScreenV2Report = () => {
  const payload = { app: BUILD_INFO.appName, version: BUILD_INFO.version, build: BUILD_INFO.build, schema: BUILD_INFO.schemaVersion, generatedAt: new Date().toISOString(), snapshot: startScreenV2Snapshot(), startScreenV2: state.startScreenV2, privacy: 'Relatório local. Não envia dados para servidor.' };
  const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `vale-tennis-start-screen-v2-${BUILD_INFO.build}.json`;
  a.click();
...
function ensureCleanStartWizardSystem() {
  ...
  const wizard = state.cleanStartWizard;
  wizard.auditLog ||= [];
  wizard.deploymentChecks ||= [];
  wizard.resetBackups ||= [];
  wizard.score ??= 100;
  wizard.lastAuditToken ??= null;
  wizard.staleBuildSeen ??= false;
  wizard.cleanStartConfirmed ??= false;
  wizard.lastResetAt ??= null;
  wizard.flags ||= { guidedReset: true, deployCheck: true, cacheClear: true, firstRunReopen: true, preserveBackup: true };
  wizard.flags.guidedReset ??= true;
  wizard.flags.deployCheck ??= true;
  wizard.flags.cacheClear ??= true;
  wizard.flags.firstRunReopen ??= true;
  wizard.flags.preserveBackup ??= true;
  return wizard;
}
function cleanStartWizardSnapshot() {
  const issues = invalidCareerIssues(state);
  const buildText = $('#buildPill')?.textContent || '';
  const overlayText = $('#buildOverlay')?.textContent || '';
  const runtimeText = $('#runtimeBuildStamp')?.textContent || '';
  const storedBuild = localStorage.getItem(LAST_SEEN_BUILD_KEY) || '';
  const saveDiag = (() => { try { return getSaveDiagnostics(); } catch { return {}; } })();
  return {
    at: new Date().toISOString(), version: BUILD_INFO.version, build: BUILD_INFO.build, label: BUILD_LABEL,
    visibleBuildOk: [buildText, overlayText, runtimeText].some(text => String(text).includes(BUILD_INFO.version) && String(text).includes(BUILD_INFO.date)),
    buildText, overlayText, runtimeText,
    storedBuild, staleBuildLikely: !!storedBuild && storedBuild !== BUILD_INFO.build,
    serviceWorker: { supported: 'serviceWorker' in navigator, controlled: !!navigator.serviceWorker?.controller },
    cacheApi: 'caches' in window,
    storageWritable: (() => { try { localStorage.setItem('vale_tennis_clean_start_probe', BUILD_INFO.build); localStorage.removeItem('vale_tennis_clean_start_probe'); return true; } catch { return false; } })(),
    invalidCareer: issues.length > 0, issues,
    modalVisible: ownerSetupModalVisible(),
    currentTab: state?.ui?.currentTab || 'dashboard',
    roster: state?.roster?.length || 0, ranking: state?.ranking?.length || 0, calendar: state?.calendar?.length || 0, money: Number(state?.academy?.money || 0),
    saveDiag, userAgent: navigator.userAgent || '', viewport: { w: window.innerWidth, h: window.innerHeight, vh: window.visualViewport?.height || window.innerHeight }
  };
}
function calculateCleanStartScore(snapshot = cleanStartWizardSnapshot()) {
  let score = 100;
  if (!snapshot.visibleBuildOk) score -= 22;
  if (snapshot.staleBuildLikely) score -= 18;
  if (!snapshot.storageWritable) score -= 16;
  if (snapshot.invalidCareer && snapshot.currentTab === 'dashboard') score -= 24;
  if (snapshot.invalidCareer && !snapshot.modalVisible) score -= 18;
  if (snapshot.roster < 1 || snapshot.ranking < 5 || snapshot.calendar < 4 || snapshot.money <= 0) score -= 14;
  return clamp(Math.round(score), 0, 100);
}
function logCleanStartWizard(title='Auditoria de início limpo', result='OK', note='', snapshot = cleanStartWizardSnapshot()) {
  const wizard = ensureCleanStartWizardSystem();
  wizard.score = calculateCleanStartScore(snapshot);
  wizard.lastAuditToken = `${BUILD_INFO.build}-${Date.now()}`;
  wizard.staleBuildSeen = !!snapshot.staleBuildLikely;
  wizard.deploymentChecks.unshift(snapshot);
  wizard.deploymentChecks = wizard.deploymentChecks.slice(0, 10);
  wizard.auditLog.unshift({ title, result, note, score: wizard.score, at: new Date().toISOString(), build: BUILD_INFO.build, snapshot });
  wizard.auditLog = wizard.auditLog.slice(0, 24);
  saveState(state);
  return wizard.score;
}
function renderCleanStartWizard() {
  const host = $('#cleanStartWizardHub');
  if (!host || !state) return;
  const wizard = ensureCleanStartWizardSystem();
  const snap = cleanStartWizardSnapshot();
  wizard.score = calculateCleanStartScore(snap);
  const checks = [
    ['Build atual visível', snap.visibleBuildOk, snap.visibleBuildOk ? BUILD_LABEL : 'O texto do topo não bate com a build JS.'],
    ['Build antiga detectada', !snap.staleBuildLikely, snap.staleBuildLikely ? `Navegador guardou ${snap.storedBuild}` : 'Sem build antiga no histórico local.'],
    ['Save local gravável', snap.storageWritable, snap.storageWritable ? 'localStorage OK' : 'O navegador bloqueou escrita local.'],
    ['Carreira inválida bloqueada', !(snap.invalidCareer && snap.currentTab === 'dashboard'), snap.invalidCareer ? 'Deve abrir criação/reparo antes do Dashboard' : 'Carreira pronta.'],
...
window.runCleanStartAudit = () => {
  const snap = cleanStartWizardSnapshot();
  const note = snap.invalidCareer ? snap.issues.join(', ') : (snap.staleBuildLikely ? `Build anterior no histórico: ${snap.storedBuild}` : 'Deploy e primeiro acesso OK.');
  const score = logCleanStartWizard('Auditoria clean start executada', scoreLabel(calculateCleanStartScore(snap)), note, snap);
  addLog(`Clean Start Wizard auditado: ${score}/100.`);
  if (snap.invalidCareer) forceOnboardingLauncher('auditoria clean start detectou carreira incompleta');
  renderCleanStartWizard();
};
window.guidedCleanFirstRun = () => {
  const wizard = ensureCleanStartWizardSystem();
  try {
    const backup = localStorage.getItem('vale_tennis_manager_save');
    if (backup) {
      const key = `vale_tennis_clean_start_backup_${BUILD_INFO.build}_${Date.now()}`;
      localStorage.setItem(key, backup);
      wizard.resetBackups.unshift({ key, at: new Date().toISOString(), build: BUILD_INFO.build });
      wizard.resetBackups = wizard.resetBackups.slice(0, 8);
    }
  } catch (error) { console.warn('Backup clean start não pôde ser salvo', error); }
  clearState();
  state = buildInitialState(content);
  migrateState();
  state.cleanStartWizard = { ...ensureCleanStartWizardSystem(), resetBackups: wizard.resetBackups || [], auditLog: wizard.auditLog || [] };
  ensureCleanStartWizardSystem().lastResetAt = new Date().toISOString();
  ensureCleanStartWizardSystem().cleanStartConfirmed = true;
  saveState(state);
...
window.cleanOldCachesAndReload = async () => {
  ...
  try {
    if ('caches' in window) {
      ...
      await Promise.all(keys.filter(key => key.startsWith('vale-tennis-')).map(key => caches.delete(key)));
      wizard.auditLog.unshift({ title:'Caches antigos removidos', result:'OK', note:`${keys.length} cache(s) encontrados.`, at:new Date().toISOString(), build: BUILD_INFO.build });
    }
    ...
    wizard.auditLog.unshift({ title:'Limpeza de cache falhou', result:'ATENÇÃO', note:String(error?.message || error), at:new Date().toISOString(), build: BUILD_INFO.build });
  }
  saveState(state);
  location.href = `./index.html?v=${BUILD_INFO.version}-${BUILD_INFO.build}&fresh=${Date.now()}#cleanstart`;
};
window.exportCleanStartReport = () => {
  const payload = { app: BUILD_INFO.appName, version: BUILD_INFO.version, build: BUILD_INFO.build, schema: BUILD_INFO.schemaVersion, generatedAt: new Date().toISOString(), snapshot: cleanStartWizardSnapshot(), cleanStartWizard: state.cleanStartWizard, privacy: 'Relatório local. Não envia dados para servidor.' };
  const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `vale-tennis-clean-start-${BUILD_INFO.build}.json`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
};

function ensureEmergencyStartControlSystem() {
  ...
  const esc = state.emergencyStartControl;
  esc.score ??= 100;
  esc.hardResetCount ??= 0;
  esc.lastResetAt ??= null;
  esc.auditLog ||= [];
  esc.routeDetected ??= null;
  esc.startupShieldVisible ??= false;
  esc.lastExportAt ??= null;
  esc.flags ||= { hardResetRoute: true, standaloneShield: true, cacheBypassUrl: true, backupBeforeReset: true, blockDashboardZeroMetrics: true };
  esc.flags.hardResetRoute ??= true;
  esc.flags.standaloneShield ??= true;
  esc.flags.cacheBypassUrl ??= true;
  esc.flags.backupBeforeReset ??= true;
  esc.flags.blockDashboardZeroMetrics ??= true;
  return esc;
}
function emergencyStartSnapshot() {
  const issues = invalidCareerIssues(state);
  const url = new URL(location.href);
  const buildText = document.body?.innerText?.match(/v\d+\.\d+\.\d+\s*•\s*\d{2}\/\d{2}\/\d{4}\s*•\s*\d{2}:\d{2}:\d{2}/)?.[0] || '';
  return {
    at: new Date().toISOString(), build: BUILD_INFO.build, version: BUILD_INFO.version,
    url: location.href, routeHardReset: url.searchParams.has('hardreset') || url.searchParams.has('freshStart') || ['newcareer','hardreset','emergencystart'].includes((location.hash||'').replace('#','')),
    buildLabelVisible: buildText.includes(BUILD_INFO.version) && buildText.includes(BUILD_INFO.time),
    issues, invalidCareer: issues.length > 0,
    dashboardZeroMetrics: Number(state?.academy?.reputation || 0) === 0 && Number(state?.academy?.money || 0) <= 0 && (state?.roster?.length || 0) === 0,
    setupModalVisible: ownerSetupModalVisible(),
    currentTab: state?.ui?.currentTab || 'dashboard',
    roster: state?.roster?.length || 0, ranking: state?.ranking?.length || 0, calendar: state?.calendar?.length || 0,
    owner: state?.academy?.owner?.name || '', academy: state?.academy?.name || '', city: state?.academy?.careerProfile?.city || '', avatar: state?.academy?.owner?.avatar || '',
    storageWritable: (() => { try { localStorage.setItem('vale-tennis-emergency-probe','1'); localStorage.removeItem('vale-tennis-emergency-probe'); return true; } catch { return false; } })(),
    serviceWorker: { supported: 'serviceWorker' in navigator, controlled: !!navigator.serviceWorker?.controller },
    cacheApi: 'caches' in window
  };
}
function calculateEmergencyStartScore(snap = emergencyStartSnapshot()) {
  let score = 100;
  if (!snap.buildLabelVisible) score -= 12;
  if (snap.invalidCareer && !snap.setupModalVisible) score -= 30;
  if (snap.dashboardZeroMetrics) score -= 24;
  score -= Math.min(24, snap.issues.length * 5);
  if (!snap.storageWritable) score -= 10;
  return clamp(Math.round(score), 0, 100);
}
function logEmergencyStart(title='Auditoria emergencial', result='OK', note='', snap = emergencyStartSnapshot()) {
  const esc = ensureEmergencyStartControlSystem();
  esc.score = calculateEmergencyStartScore(snap);
  esc.routeDetected = snap.routeHardReset ? location.href : esc.routeDetected;
  esc.auditLog.unshift({ title, result, note, score: esc.score, at: new Date().toISOString(), build: BUILD_INFO.build, snapshot: snap });
  esc.auditLog = esc.auditLog.slice(0, 24);
  saveState(state);
  return esc.score;
}
function showStandaloneStartupShield(reason='carreira inválida') {
  if (careerIsPlayableAndConfigured(state)) return;
  let shield = document.getElementById('emergencyStartupShield');
  if (!shield) {
    shield = document.createElement('div');
    shield.id = 'emergencyStartupShield';
    shield.className = 'emergency-startup-shield';
...
function hideStandaloneStartupShield() {
  document.getElementById('emergencyStartupShield')?.classList.add('hidden');
  if (state?.emergencyStartControl) state.emergencyStartControl.startupShieldVisible = false;
}
window.hideStandaloneStartupShield = hideStandaloneStartupShield;
function applyEmergencyStartRoute() {
  const url = new URL(location.href);
  const hash = (location.hash || '').replace('#','');
  if (url.searchParams.has('hardreset') || url.searchParams.has('freshStart') || hash === 'newcareer' || hash === 'hardreset') {
    ensureEmergencyStartControlSystem().routeDetected = location.href;
    setTimeout(() => window.hardResetFirstRun?.('rota hardreset/freshStart'), 120);
    return;
  }
  if (!careerIsPlayableAndConfigured(state)) {
...
function renderEmergencyStartControl() {
  const host = $('#emergencyStartControlHub');
  if (!host || !state) return;
  const esc = ensureEmergencyStartControlSystem();
  const snap = emergencyStartSnapshot();
  esc.score = calculateEmergencyStartScore(snap);
  const checks = [
    ['Build atual exibida', snap.buildLabelVisible, snap.buildLabelVisible ? BUILD_LABEL : 'Topo não mostra a build JS atual.'],
    ['Dashboard vazio bloqueado', !(snap.invalidCareer && snap.currentTab === 'dashboard'), snap.invalidCareer ? `Tab atual: ${snap.currentTab}` : 'Carreira liberada.'],
    ['Criação visível quando inválida', !snap.invalidCareer || snap.setupModalVisible || SETUP_SAFE_TABS.has(snap.currentTab), snap.setupModalVisible ? 'Modal aberto' : 'Use Abrir criação.'],
    ['Base jogável', snap.roster > 0 && snap.ranking >= 5 && snap.calendar >= 4, `${snap.roster} atletas • ${snap.ranking} ranking • ${snap.calendar} eventos`],
...
window.hardResetFirstRun = async (reason='hard reset guiado') => {
  ...
  try {
    const previous = localStorage.getItem('vale_tennis_manager_save');
    if (previous) localStorage.setItem(`vale_tennis_hard_reset_backup_${BUILD_INFO.build}_${Date.now()}`, previous);
  } catch (error) { console.warn('Backup hard reset indisponível', error); }
  clearState();
  state = buildInitialState(content);
  migrateState();
  state.flags ||= {};
  state.flags.ownerSetupComplete = false;
  state.ui = { currentTab:'emergencystart', lastStableTab:'emergencystart' };
  state.emergencyStartControl = { ...ensureEmergencyStartControlSystem(), hardResetCount: (esc.hardResetCount || 0) + 1, lastResetAt: new Date().toISOString(), auditLog: esc.auditLog || [] };
  logEmergencyStart('Hard reset de primeiro acesso', 'OK', reason);
  saveState(state);
  render();
  switchTab('emergencystart');
  forceOnboardingLauncher(reason);
  setTimeout(() => { if (!ownerSetupModalVisible()) showStandaloneStartupShield(reason); }, 650);
};
window.clearCachesThenFreshStart = async () => {
  try {
    if ('caches' in window) {
      const keys = await caches.keys();
      await Promise.all(keys.filter(key => key.startsWith('vale-tennis-')).map(key => caches.delete(key)));
  ...
  } catch (error) { console.warn('Limpeza emergencial de cache falhou', error); }
  location.href = `./index.html?v=${BUILD_INFO.version}-${BUILD_INFO.build}&hardreset=1&fresh=${Date.now()}#emergencystart`;
};
window.exportEmergencyStartReport = () => {
  const esc = ensureEmergencyStartControlSystem();
  esc.lastExportAt = new Date().toISOString();
  const payload = { app: BUILD_INFO.appName, version: BUILD_INFO.version, build: BUILD_INFO.build, schema: BUILD_INFO.schemaVersion, generatedAt: new Date().toISOString(), snapshot: emergencyStartSnapshot(), emergencyStartControl: esc, privacy: 'Relatório local. Não envia dados para servidor.' };
  const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `vale-tennis-emergency-start-${BUILD_INFO.build}.json`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
};
...
function preserveRuntimePreferences(fresh, previous) {
  ...
  const preserve = ['mobileUX','inputReliability','accessibilityReadability','localizationStore','releaseNotesHelp','onboardingReliability','careerCreationUX','qualityPolish','releaseHardening','performanceDelivery','qaAutomation','browserCompatibility','mandatoryCareerGate','forcedOnboardingGate','cleanStartWizard','emergencyStartControl','startScreenV2'];
...
function handleStartupHash() {
  ...
  if (hash === 'start' || hash === 'startscreen') return launchStartScreenV2('atalho #' + hash, true);
...
async function boot() {
  ...
  setTimeout(() => { try { window.runCleanStartAudit?.(); } catch (error) { console.warn('Clean Start Wizard boot fallback', error); } }, 520);
  startCourtAnimation();
  drawCourt();
  render();
  hydrateAssetImages();
...
function switchTab(tab) {
  ...
  applyMobileUXRuntime();
  if (tab === 'match' || tab === 'broadcast2d') drawCourt();
  if (window.innerWidth < 1100) scrollToAppTop(true);
}


function visualSceneForTab(tab='dashboard') {
  const map = { startscreen: 'office', dashboard: 'office', facilitiespro: 'training', scoutingworld: 'market', visual: state.visualAcademy?.activeScene || 'office', roster: 'market', career: 'office', training: 'training', microcycle: 'training', calendar: 'calendar', newsroom: 'calendar', mobileux: 'office', economy: 'office', legacy: 'office', release: 'office', delivery: 'office', qa: 'office', compat: 'office', onboarding: 'office', cacheguard: 'office', setupverify: 'office', initialgate: 'office', runtimeproof: 'office', facilitiespro: 'training', input: 'office', a11y: 'office', broadcast2d: 'broadcast', seasonboard: 'office', relationships: 'office', match: 'broadcast', market: 'market', staff: 'medical', ranking: 'calendar', adminhint: 'office' };
...
const MICRO_LOAD_POLICIES = {
  conservative: { label: 'Conservador', desc: 'Preserva saúde, reduz carga e prioriza recuperação antes de torneios longos.', load: -10, recovery: 12, risk: -8, perf: -1 },
  balanced: { label: 'Equilibrado', desc: 'Combina evolução técnica, preparação física e recuperação sem extremos.', load: 0, recovery: 5, risk: -2, perf: 1 },
  aggressive: { label: 'Agressivo', desc: 'Aumenta estímulo competitivo e evolução, mas exige monitoramento médico.', load: 12, recovery: -2, risk: 7, perf: 4 },
  peak: { label: 'Pico competitivo', desc: 'Semana curta de ativação para torneio: menos volume, mais velocidade e confiança.', load: -4, recovery: 4, risk: -4, perf: 5 }
};
const MICRO_RECOVERY_MODES = {
  standard: { label: 'Padrão', desc: 'Recuperação normal do staff e instalações.', recovery: 0, risk: 0 },
  medical: { label: 'Médico preventivo', desc: 'Fisioterapia e monitoramento ativo para reduzir risco de lesão.', recovery: 8, risk: -7 },
  mental: { label: 'Mental e pressão', desc: 'Psicologia esportiva, redução de pressão e moral mais estável.', recovery: 3, risk: -2, pressure: -6 },
  regeneration: { label: 'Regenerativo', desc: 'Semana de carga baixa, sono, mobilidade e retorno gradual.', recovery: 14, risk: -11, pressure: -3 }
};
const MICRO_FOCUS_OPTIONS = {
  balanced: { label: 'Equilíbrio geral', trainingFocus: 'balanced', intensity: 'moderate', attrs: ['forehand','backhand','serve','return'] },
  serveReturn: { label: 'Saque + devolução', trainingFocus: 'technique', intensity: 'moderate', attrs: ['serve','return','composure'] },
  clayEndurance: { label: 'Saibro e resistência', trainingFocus: 'fitness', intensity: 'intense', attrs: ['stamina','consistency','speed'] },
  matchSharpness: { label: 'Ritmo de jogo', trainingFocus: 'match', intensity: 'moderate', attrs: ['tacticalIQ','composure','return'] },
  recovery: { label: 'Recuperação total', trainingFocus: 'recovery', intensity: 'light', attrs: [] }
};
...
function ensureTrainingMicrocycle(){
  state.trainingMicrocycle ||= { score: 82, loadPolicy: 'balanced', recoveryPriority: 'standard', nextWeekFocus: 'balanced', injuryPrevention: true, athleteLoadMap: {}, weeklyLog: [], auditLog: [], lastProcessedToken: null, lastAuditToken: null, flags: { periodization: true, injuryPrevention: true, facilitySynergy: true, matchEngineImpact: true, mobileLoadBoard: true } };
  const mc = state.trainingMicrocycle;
  mc.loadPolicy ||= 'balanced';
  mc.recoveryPriority ||= 'standard';
  mc.nextWeekFocus ||= 'balanced';
  mc.injuryPrevention ??= true;
  mc.athleteLoadMap ||= {};
  mc.weeklyLog ||= [];
  mc.auditLog ||= [];
  mc.flags ||= { periodization: true, injuryPrevention: true, facilitySynergy: true, matchEngineImpact: true, mobileLoadBoard: true };
  state.roster.forEach(player => {
    const existing = mc.athleteLoadMap[player.id] || {};
    const acute = Math.round(clamp(existing.acuteLoad ?? ((player.fatigue || 0) + (100 - (player.health || 100)) * 0.38 + (player.injuredWeeks || 0) * 18), 0, 120));
    const chronic = Math.round(clamp(existing.chronicLoad ?? 42, 10, 120));
    const readiness = Math.round(clamp(existing.readiness ?? ((player.health || 100) - (player.fatigue || 0) * 0.42 - (player.injuredWeeks || 0) * 20 + (player.morale || 70) * 0.08), 0, 100));
    const injuryRisk = Math.round(clamp(existing.injuryRisk ?? microcycleInjuryRisk(player, acute, chronic), 0, 100));
    mc.athleteLoadMap[player.id] = { acuteLoad: acute, chronicLoad: chronic, readiness, injuryRisk, lastFocus: existing.lastFocus || 'balanced', lastDecision: existing.lastDecision || 'monitorar' };
  });
  return mc;
}
function microcycleInjuryRisk(player, acute=null, chronic=null){
  const load = acute ?? ((player.fatigue || 0) + (100 - (player.health || 100)) * 0.35);
  const base = (player.injuryProneness || 50) * 0.32 + load * 0.52 - (chronic || 42) * 0.12;
  const medical = (state.academy?.facilities?.medical || 1) * 1.8 + Math.abs(Math.min(0, getStaffBonus('Fisioterapeuta','injury') + getStaffBonus('Nutricionista','injury')));
  return clamp(base - medical + (player.injuredWeeks || 0) * 26, 2, 96);
}
function microcycleSnapshot(){
  const mc = ensureTrainingMicrocycle();
  const values = state.roster.map(player => mc.athleteLoadMap[player.id] || {});
  const avgReady = Math.round(values.reduce((sum,row)=>sum+(row.readiness||0),0)/Math.max(1,values.length));
  const avgRisk = Math.round(values.reduce((sum,row)=>sum+(row.injuryRisk||0),0)/Math.max(1,values.length));
  const highRisk = values.filter(row => (row.injuryRisk || 0) >= 55).length;
  const peakReady = values.filter(row => (row.readiness || 0) >= 74).length;
  const score = Math.round(clamp(100 - avgRisk * 0.62 + avgReady * 0.22 - highRisk * 4 + peakReady * 2, 0, 100));
  mc.score = score;
  return { avgReady, avgRisk, highRisk, peakReady, score };
}
function microcycleRecommendation(player){
  const mc = ensureTrainingMicrocycle();
  const row = mc.athleteLoadMap[player.id] || {};
  if ((player.injuredWeeks || 0) > 0) return { focus:'recovery', intensity:'light', decision:'lesionado: recuperação obrigatória' };
  if ((row.injuryRisk || 0) >= 62 || (row.readiness || 0) <= 42) return { focus:'recovery', intensity:'light', decision:'risco alto: reduzir carga' };
  if (mc.loadPolicy === 'peak') return { focus:'match', intensity:'light', decision:'pico: ativação competitiva' };
  const focus = MICRO_FOCUS_OPTIONS[mc.nextWeekFocus] || MICRO_FOCUS_OPTIONS.balanced;
  const policy = MICRO_LOAD_POLICIES[mc.loadPolicy] || MICRO_LOAD_POLICIES.balanced;
  const intensity = policy.load > 8 && (row.injuryRisk || 0) < 42 ? 'intense' : focus.intensity;
  return { focus: focus.trainingFocus, intensity, decision: `${focus.label} • ${MICRO_LOAD_POLICIES[mc.loadPolicy].label}` };
}
function renderTrainingMicrocycle(){
  const host = $('#trainingMicrocycleHub');
  if(!host) return;
  ensureTrainingLab();
  const mc = ensureTrainingMicrocycle();
  const snap = microcycleSnapshot();
  const policyOptions = Object.entries(MICRO_LOAD_POLICIES).map(([key,val])=>`<option value="${key}" ${mc.loadPolicy===key?'selected':''}>${val.label}</option>`).join('');
  const recoveryOptions = Object.entries(MICRO_RECOVERY_MODES).map(([key,val])=>`<option value="${key}" ${mc.recoveryPriority===key?'selected':''}>${val.label}</option>`).join('');
  const focusOptions = Object.entries(MICRO_FOCUS_OPTIONS).map(([key,val])=>`<option value="${key}" ${mc.nextWeekFocus===key?'selected':''}>${val.label}</option>`).join('');
  const rows = state.roster.map(player => {
    const row = mc.athleteLoadMap[player.id] || {};
    const rec = microcycleRecommendation(player);
    const riskClass = (row.injuryRisk || 0) >= 60 ? 'danger' : (row.injuryRisk || 0) >= 38 ? 'warn' : 'ok';
    return `<article class="panel-card micro-athlete-card">
      <div class="training-player-head">${avatarImg(avatarForPlayer(player.name),'avatar-img',player.name)}<div><h4>${player.name}</h4><p class="muted">Readiness ${row.readiness || 0} • carga aguda ${row.acuteLoad || 0} • crônica ${row.chronicLoad || 0}</p></div><span class="risk-chip ${riskClass}">Risco ${Math.round(row.injuryRisk || 0)}%</span></div>
      <div class="microcycle-bars"><span>Prontidão <i style="width:${clamp(row.readiness||0,2,100)}%"></i></span><span>Carga <i style="width:${clamp(row.acuteLoad||0,2,100)}%"></i></span></div>
      <div class="scout-row"><strong>Recomendação</strong><span>${TRAINING_FOCUS[rec.focus]?.label || rec.focus} / ${TRAINING_INTENSITY[rec.intensity]?.label || rec.intensity} • ${rec.decision}</span></div>
    </article>`;
  }).join('');
  host.innerHTML = `<div class="training-summary-grid microcycle-summary">
    <article class="stat-card"><span>Score microciclo</span><strong>${snap.score}</strong></article>
    <article class="stat-card"><span>Readiness média</span><strong>${snap.avgReady}</strong></article>
    <article class="stat-card"><span>Risco médio</span><strong>${snap.avgRisk}%</strong></article>
    <article class="stat-card"><span>Alto risco</span><strong>${snap.highRisk}</strong></article>
  </div>
  <article class="panel-card training-control-card microcycle-control">
    <div><h4>Direção da semana</h4><p class="muted">O microciclo ajusta carga, recuperação e plano individual antes do Training Lab processar a semana.</p></div>
    <div class="training-fields"><label>Política de carga<select onchange="window.setMicrocycleOption('loadPolicy',this.value)">${policyOptions}</select></label><label>Recuperação<select onchange="window.setMicrocycleOption('recoveryPriority',this.value)">${recoveryOptions}</select></label><label>Foco principal<select onchange="window.setMicrocycleOption('nextWeekFocus',this.value)">${focusOptions}</select></label></div>
    <div class="form-actions"><button class="btn-primary" onclick="window.applyMicrocyclePlan()">Aplicar plano recomendado</button><button class="btn-secondary" onclick="window.runMicrocycleAudit()">Auditar risco</button><button class="btn-secondary" onclick="window.exportMicrocycleReport()">Exportar relatório</button></div>
  </article>
  <div class="training-plan-grid microcycle-grid">${rows}</div>
  <article class="panel-card training-report-card"><div class="panel-title-row"><h4>Log do microciclo</h4></div><div class="list-block">${(mc.weeklyLog||[]).slice(0,8).map(item=>`<div class="list-item"><span>${item.title || item}</span><small>${item.detail || ''}</small></div>`).join('') || '<div class="list-item"><span>Nenhuma semana processada pelo microciclo ainda.</span></div>'}</div></article>`;
  hydrateAssetImages();
}
window.setMicrocycleOption = (key,value) => { const mc = ensureTrainingMicrocycle(); if(!['loadPolicy','recoveryPriority','nextWeekFocus'].includes(key)) return; mc[key] = value; mc.auditLog.unshift({ title:'Opção do microciclo alterada', detail:`${key}: ${value}`, week:state.academy.week, season:state.academy.season, build:BUILD_INFO.build }); saveState(state); renderTrainingMicrocycle(); };
window.applyMicrocyclePlan = () => {
  const mc = ensureTrainingMicrocycle(); ensureTrainingLab();
  state.roster.forEach(player => { const rec = microcycleRecommendation(player); state.trainingLab.plans[player.id] = { focus: rec.focus, intensity: rec.intensity }; mc.athleteLoadMap[player.id].lastDecision = rec.decision; mc.athleteLoadMap[player.id].lastFocus = rec.focus; });
  mc.weeklyLog.unshift({ title:'Plano recomendado aplicado ao Training Lab', detail:`${MICRO_LOAD_POLICIES[mc.loadPolicy]?.label || mc.loadPolicy} • ${MICRO_FOCUS_OPTIONS[mc.nextWeekFocus]?.label || mc.nextWeekFocus}`, week:state.academy.week, season:state.academy.season });
  addLog('Microciclo avançado aplicou planos individuais ao Training Lab.');
  saveState(state); render();
};
window.runMicrocycleAudit = () => { const mc = ensureTrainingMicrocycle(); const snap = microcycleSnapshot(); mc.lastAuditToken = `${BUILD_INFO.build}-${Date.now()}`; mc.auditLog.unshift({ title:'Auditoria de microciclo', detail:`Score ${snap.score}, risco médio ${snap.avgRisk}%, ${snap.highRisk} atletas em alto risco.`, week:state.academy.week, season:state.academy.season, build:BUILD_INFO.build }); mc.weeklyLog.unshift({ title:'Auditoria executada', detail:`Readiness ${snap.avgReady} • risco ${snap.avgRisk}% • alto risco ${snap.highRisk}`, week:state.academy.week, season:state.academy.season }); saveState(state); renderTrainingMicrocycle(); };
function processTrainingMicrocycleWeek(){
  const mc = ensureTrainingMicrocycle(); const token = `${state.academy.season}-${state.academy.week}`;
  if(mc.lastProcessedToken === token) return;
  const policy = MICRO_LOAD_POLICIES[mc.loadPolicy] || MICRO_LOAD_POLICIES.balanced;
  const recovery = MICRO_RECOVERY_MODES[mc.recoveryPriority] || MICRO_RECOVERY_MODES.standard;
  const facilityRecovery = (state.academy?.facilities?.medical || 1) * 1.3 + (state.academy?.facilities?.gym || 1) * 0.6 + (state.academy?.facilities?.analytics || 0) * 0.7;
  const log = [];
  state.roster.forEach(player => {
    const row = mc.athleteLoadMap[player.id] ||= { acuteLoad: 35, chronicLoad: 42, readiness: 70, injuryRisk: 12, lastFocus:'balanced', lastDecision:'monitorar' };
    const rec = microcycleRecommendation(player);
    state.trainingLab ||= { plans: {} }; state.trainingLab.plans ||= {};
    if(mc.injuryPrevention && ((row.injuryRisk || 0) >= 62 || (player.injuredWeeks || 0) > 0)) state.trainingLab.plans[player.id] = { focus:'recovery', intensity:'light' };
    else state.trainingLab.plans[player.id] ||= { focus: rec.focus, intensity: rec.intensity };
    const acuteDelta = policy.load + (state.trainingLab.plans[player.id]?.intensity === 'intense' ? 8 : state.trainingLab.plans[player.id]?.intensity === 'light' ? -8 : 0) - recovery.recovery * 0.35;
    row.acuteLoad = Math.round(clamp((row.acuteLoad || 35) * 0.62 + Math.max(0, (player.fatigue || 0) + acuteDelta) * 0.38, 0, 120));
    row.chronicLoad = Math.round(clamp((row.chronicLoad || 42) * 0.82 + row.acuteLoad * 0.18, 8, 120));
    row.readiness = Math.round(clamp((player.health || 100) - row.acuteLoad * 0.28 + recovery.recovery + facilityRecovery + ((player.morale || 70) - 60) * 0.08 + (recovery.pressure || 0) * -0.15, 0, 100));
    row.injuryRisk = Math.round(clamp(microcycleInjuryRisk(player, row.acuteLoad, row.chronicLoad) + policy.risk + recovery.risk, 1, 96));
    row.lastFocus = state.trainingLab.plans[player.id]?.focus || rec.focus;
    row.lastDecision = rec.decision;
    player.readiness = row.readiness;
    player.fatigue = Math.max(0, (player.fatigue || 0) + Math.max(-10, policy.load * 0.12 - recovery.recovery * 0.16));
    player.health = Math.min(100, (player.health || 100) + recovery.recovery * 0.08 + facilityRecovery * 0.06);
    if (row.injuryRisk >= 70) maybeInjure(player, 4 + row.injuryRisk * 0.05);
    log.push(`${player.name}: prontidão ${row.readiness}, risco ${row.injuryRisk}%, decisão ${row.lastDecision}.`);
  });
  mc.lastProcessedToken = token;
  mc.weeklyLog.unshift({ title:`Microciclo processado na semana ${state.academy.week}`, detail:log.slice(0,3).join(' • '), week:state.academy.week, season:state.academy.season, build:BUILD_INFO.build });
  mc.weeklyLog = mc.weeklyLog.slice(0,16);
}
window.exportMicrocycleReport = () => { const mc = ensureTrainingMicrocycle(); const payload = { app:BUILD_INFO.appName, version:BUILD_INFO.version, build:BUILD_INFO.build, schema:BUILD_INFO.schemaVersion, generatedAt:new Date().toISOString(), snapshot:microcycleSnapshot(), settings:{ loadPolicy:mc.loadPolicy, recoveryPriority:mc.recoveryPriority, nextWeekFocus:mc.nextWeekFocus, injuryPrevention:mc.injuryPrevention }, athletes:state.roster.map(p=>({ id:p.id, name:p.name, fatigue:p.fatigue, health:p.health, readiness:p.readiness, injuredWeeks:p.injuredWeeks, load:mc.athleteLoadMap[p.id] })), weeklyLog:mc.weeklyLog, privacy:'Relatório local. Nenhum dado é enviado para servidor.' }; try { const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href=url; link.download=`vale-tennis-microcycle-${BUILD_INFO.version}-${BUILD_INFO.build}.json`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); } catch(error) { console.info('Microcycle report', payload, error); } mc.lastAuditToken = `${BUILD_INFO.build}-${Date.now()}`; mc.auditLog.unshift({ title:'Relatório de microciclo exportado', detail:'JSON local gerado', week:state.academy.week, season:state.academy.season, build:BUILD_INFO.build }); saveState(state); renderTrainingMicrocycle(); };
...
function playPoint() {
  ...
  try {
    ...
    recordMatchVisualPoint(point, state.match);
...
function ensureMatchVisual2D(save=false) {
  if (!state) return { score: 88, cameraMode: 'broadcast', overlayMode: 'standard', ballTrail: true, shotMap: [], auditLog: [], flags: {} };
  ...
  const mv = state.matchVisual2D;
  mv.score ??= 88;
  mv.cameraMode ||= 'broadcast';
  mv.overlayMode ||= 'standard';
  mv.ballTrail ??= true;
  mv.shotMap ||= [];
  mv.auditLog ||= [];
  mv.flags ||= { cinematicCourt: true, shotMapLive: true, tacticalZones: true, mobileBroadcast: true };
  if (save) saveState(state);
  return mv;
}
function matchVisualScore() {
  const mv = ensureMatchVisual2D();
  const hasMatch = !!state.match;
  const points = mv.shotMap?.length || 0;
  const hasCanvas = !!document.getElementById('matchCanvas');
  return clamp(70 + (hasCanvas ? 8 : 0) + (hasMatch ? 7 : 0) + Math.min(10, points) + (mv.ballTrail ? 3 : 0), 0, 100);
}
function renderMatchVisual2D() {
  const host = $('#matchVisual2DHub');
  if (!host || !state) return;
  const mv = ensureMatchVisual2D();
  mv.score = matchVisualScore();
  const match = state.match;
  const shotRows = (mv.shotMap || []).slice(-8).reverse().map((p, i) => `<div class="list-item"><span>${i+1}. ${escapeHtml(p.type || 'Ponto')} • ${escapeHtml(p.winner === 'player' ? 'Seu atleta' : 'Adversário')}</span><strong>${p.speed || '—'} km/h</strong></div>`).join('') || '<div class="empty-state">Jogue pontos no Match Center para popular o mapa 2D.</div>';
  const cameraButtons = Object.entries(MATCH_VISUAL_CAMERAS).map(([key, item]) => `<button class="mini-btn ${mv.cameraMode === key ? 'active' : ''}" onclick="window.setMatchVisualCamera('${key}')">${item.label}</button>`).join('');
  const overlayButtons = Object.entries(MATCH_VISUAL_OVERLAYS).map(([key, item]) => `<button class="mini-btn ${mv.overlayMode === key ? 'active' : ''}" onclick="window.setMatchVisualOverlay('${key}')">${item.label}</button>`).join('');
  const current = MATCH_VISUAL_CAMERAS[mv.cameraMode] || MATCH_VISUAL_CAMERAS.broadcast;
  host.innerHTML = `
    <section class="match-visual-hero">
      <div><p class="eyebrow">Match Visual 2D Broadcast Upgrade</p><h2>Transmissão 2D com leitura tática</h2><p>${current.desc}</p></div>
      <div class="release-score-card"><span>Prontidão visual</span><strong>${mv.score}/100</strong><small>${BUILD_LABEL}</small></div>
    </section>
    <section class="release-grid match-visual-grid">
      <article class="panel-card"><h4>Câmera</h4><div class="tag-row">${cameraButtons}</div><p class="muted">Modo atual: ${current.label}</p></article>
      <article class="panel-card"><h4>Overlay</h4><div class="tag-row">${overlayButtons}</div><p class="muted">Heatmap, zonas e HUD mobile usam os últimos pontos simulados.</p></article>
      <article class="panel-card"><h4>Status da partida</h4><div class="list-block"><div class="list-item"><span>Partida ativa</span><strong>${match ? 'Sim' : 'Não'}</strong></div><div class="list-item"><span>Pontos no mapa</span><strong>${mv.shotMap.length}</strong></div><div class="list-item"><span>Rastro da bola</span><strong>${mv.ballTrail ? 'Ligado' : 'Desligado'}</strong></div><div class="list-item"><span>Overlay</span><strong>${MATCH_VISUAL_OVERLAYS[mv.overlayMode]?.label || mv.overlayMode}</strong></div></div></article>
      <article class="panel-card"><h4>Controles rápidos</h4><div class="tag-row"><button class="btn-primary" onclick="window.openMatch2DView()">Abrir partida 2D</button><button class="btn-secondary" onclick="window.toggleMatchBallTrail()">Alternar rastro</button><button class="btn-ghost" onclick="window.runMatchVisualAudit()">Auditar</button><button class="btn-ghost" onclick="window.exportMatchVisualReport()">Exportar</button></div></article>
    </section>
    <section class="release-grid"><article class="panel-card"><h4>Mapa dos últimos pontos</h4><div class="shot-map-mini">${(mv.shotMap || []).slice(-28).map(p => `<i title="${escapeHtml(p.type || 'ponto')}" class="${p.winner === 'player' ? 'player' : 'opponent'}" style="left:${p.xPct || 50}%;top:${p.yPct || 50}%"></i>`).join('')}</div></article><article class="panel-card"><h4>Replay textual</h4><div class="list-block">${shotRows}</div></article></section>`;
}
function recordMatchVisualPoint(point, match) {
  const mv = ensureMatchVisual2D();
  if (!point || !match) return;
  const side = point.winner === 'player' ? 1 : -1;
  const baseX = point.direction === 'aberto' ? 68 : point.direction === 'T' ? 50 : point.direction === 'corpo' ? 42 : 56;
  const rallyShift = clamp((point.rally || 0) * 1.8, 0, 26);
  const xPct = clamp(baseX + (Math.random() * 18 - 9), 12, 88);
  const yPct = clamp((point.winner === 'player' ? 35 : 65) + side * (point.type === 'Ace' ? -10 : point.type === 'Winner' ? -rallyShift * 0.4 : rallyShift * 0.25), 14, 86);
  mv.shotMap.push({ type: point.type, winner: point.winner, speed: point.speed || match.lastServeSpeed || 0, rally: point.rally || 0, direction: point.direction || 'misto', xPct: Math.round(xPct), yPct: Math.round(yPct), score: scoreSnapshot(match), at: new Date().toISOString(), build: BUILD_INFO.build });
  mv.shotMap = mv.shotMap.slice(-90);
}
function drawMatchVisualOverlay(ctx, w, h, match, p1, p2, bx, by) {
  const mv = ensureMatchVisual2D();
  if (!mv || mv.overlayMode === 'cinematic') return;
  ctx.save();
  if (mv.overlayMode === 'tactical' || mv.cameraMode === 'coach' || mv.cameraMode === 'tactical') {
    ctx.fillStyle = 'rgba(255,255,255,0.06)';
    for (let i = 1; i < 4; i++) ctx.fillRect(w * (0.12 + i * 0.19), h * 0.12, 2, h * 0.76);
    ctx.fillStyle = 'rgba(111, 231, 255, 0.12)';
    ctx.fillRect(w * 0.12, h * 0.62, w * 0.76, h * 0.26);
    ctx.fillStyle = 'rgba(255, 178, 87, 0.10)';
    ctx.fillRect(w * 0.12, h * 0.12, w * 0.76, h * 0.26);
  }
  const points = (mv.shotMap || []).slice(-40);
  if (points.length && (mv.overlayMode === 'heatmap' || mv.cameraMode === 'tactical')) {
    points.forEach((p, idx) => {
      ctx.fillStyle = p.winner === 'player' ? `rgba(120,255,160,${0.10 + idx / 420})` : `rgba(255,120,150,${0.10 + idx / 420})`;
      ctx.beginPath();
      ctx.arc(w * (p.xPct || 50) / 100, h * (p.yPct || 50) / 100, Math.max(8, w * 0.012), 0, Math.PI * 2);
      ctx.fill();
    });
  }
  if (mv.ballTrail) {
    const pointsTrail = [p1, { x: bx, y: by }, p2];
    ctx.strokeStyle = 'rgba(255,255,255,0.34)';
    ctx.lineWidth = Math.max(2, w * 0.003);
    ctx.beginPath();
    ctx.moveTo(pointsTrail[0].x, pointsTrail[0].y);
    ctx.quadraticCurveTo(pointsTrail[1].x, pointsTrail[1].y - h * 0.1, pointsTrail[2].x, pointsTrail[2].y);
    ctx.stroke();
  }
  ctx.restore();
}
window.setMatchVisualCamera = (mode) => { const mv = ensureMatchVisual2D(); if (!MATCH_VISUAL_CAMERAS[mode]) return; mv.cameraMode = mode; mv.auditLog.unshift({ title:'Câmera alterada', result:MATCH_VISUAL_CAMERAS[mode].label, at:new Date().toISOString(), build:BUILD_INFO.build }); saveState(state); drawCourt(); renderMatchVisual2D(); };
window.setMatchVisualOverlay = (mode) => { const mv = ensureMatchVisual2D(); if (!MATCH_VISUAL_OVERLAYS[mode]) return; mv.overlayMode = mode; mv.auditLog.unshift({ title:'Overlay alterado', result:MATCH_VISUAL_OVERLAYS[mode].label, at:new Date().toISOString(), build:BUILD_INFO.build }); saveState(state); drawCourt(); renderMatchVisual2D(); };
window.toggleMatchBallTrail = () => { const mv = ensureMatchVisual2D(); mv.ballTrail = !mv.ballTrail; mv.auditLog.unshift({ title:'Rastro da bola', result:mv.ballTrail ? 'Ligado' : 'Desligado', at:new Date().toISOString(), build:BUILD_INFO.build }); saveState(state); drawCourt(); renderMatchVisual2D(); };
window.openMatch2DView = () => { switchTab('match'); drawCourt(); renderMatch(); renderMatchVisual2D(); };
window.runMatchVisualAudit = () => { const mv = ensureMatchVisual2D(); mv.score = matchVisualScore(); mv.lastAuditToken = `${BUILD_INFO.build}-${Date.now()}`; mv.auditLog.unshift({ title:'Auditoria Broadcast 2D', result:`${mv.score}/100`, note:`Canvas ${canvas ? 'OK' : 'ausente'} • pontos ${mv.shotMap.length} • câmera ${mv.cameraMode}.`, at:new Date().toISOString(), build:BUILD_INFO.build }); mv.auditLog = mv.auditLog.slice(0,30); saveState(state); renderMatchVisual2D(); addLog(`Broadcast 2D auditado: ${mv.score}/100.`); };
window.exportMatchVisualReport = () => { const mv = ensureMatchVisual2D(); const payload = { app:BUILD_INFO.appName, version:BUILD_INFO.version, build:BUILD_INFO.build, schema:BUILD_INFO.schemaVersion, generatedAt:new Date().toISOString(), cameraMode:mv.cameraMode, overlayMode:mv.overlayMode, score:matchVisualScore(), shotMap:mv.shotMap, auditLog:mv.auditLog, match: state.match ? { event:state.match.event?.name, player:state.match.playerName, opponent:state.match.opponentName, score:scoreSnapshot(state.match) } : null, privacy:'Relatório local. Nenhum dado é enviado para servidor.' }; try { const blob = new Blob([JSON.stringify(payload,null,2)], { type:'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `vale-tennis-broadcast2d-${BUILD_INFO.version}-${BUILD_INFO.build}.json`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); } catch(error) { console.info('Match Visual 2D report', payload, error); } mv.auditLog.unshift({ title:'Relatório Broadcast 2D exportado', result:'JSON', at:payload.generatedAt, build:BUILD_INFO.build }); saveState(state); renderMatchVisual2D(); };
...
function drawCourt(lastWinner = null) {
  ...
  drawMatchVisualOverlay(ctx, w, h, match, p1, p2, bx, by);
  ...
  ctx.fillText(`Estratégia ${currentStrategy.toUpperCase()} • 2D ${ensureMatchVisual2D().cameraMode.toUpperCase()} • ${autoLabel}`, 30, 61);
  if (match?.lastPoint) {
...
window.exportBootRecoveryDiagnostic = () => {
  const payload = {
    app: BUILD_INFO.appName, version: BUILD_INFO.version, build: BUILD_INFO.build, schema: BUILD_INFO.schemaVersion,
    generatedAt: new Date().toISOString(), moduleLoaded: !!window.__valeTennisModuleLoaded, bootReady: !!window.__valeTennisBootReady,
    currentTab: state?.ui?.currentTab, playable: state ? careerIsPlayableAndConfigured(state) : false,
    contentFallback: content?.manifest?.fallback || false, contentSource: content?.worldTourSource || 'unknown',
    roster: state?.roster?.length || 0, ranking: state?.ranking?.length || 0, calendar: state?.calendar?.length || 0,
    ownerSetupComplete: !!state?.flags?.ownerSetupComplete,
    userAgent: navigator.userAgent,
  ...
  try {
    ...
    a.href = url; a.download = `vale-tennis-boot-recovery-${BUILD_INFO.version}-${BUILD_INFO.build}.json`;
    document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  } catch (error) { console.info('Boot recovery diagnostic', payload, error); }
  return payload;
};
