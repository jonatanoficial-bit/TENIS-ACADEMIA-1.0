...
export function migrateSave(data) {
  ...
  state.startScreenV2 ||= { score: 100, visits: 0, lastReason: null, lastAction: null, dismissedForBuild: null, auditLog: [], flags: { premiumLanding: true, careerCreator20: true, blockDashboardUntilValid: true, continueRequiresValidCareer: true, guidedRecovery: true } };
  state.startScreenV2.score ??= 100;
  state.startScreenV2.visits ??= 0;
  state.startScreenV2.lastReason ??= null;
  state.startScreenV2.lastAction ??= null;
  state.startScreenV2.dismissedForBuild ??= null;
  state.startScreenV2.auditLog ||= [];
  state.startScreenV2.flags ||= { premiumLanding: true, careerCreator20: true, blockDashboardUntilValid: true, continueRequiresValidCareer: true, guidedRecovery: true };
  state.startScreenV2.flags.premiumLanding ??= true;
  state.startScreenV2.flags.careerCreator20 ??= true;
  ...
  state.seasonBoardPressure.score ??= 72;
  state.seasonBoardPressure.ambition ||= 'balanced';
  state.seasonBoardPressure.boardConfidence ??= 68;
  state.seasonBoardPressure.pressure ??= 34;
  state.seasonBoardPressure.lastProcessedToken ??= null;
  state.seasonBoardPressure.lastAuditToken ??= null;
  state.seasonBoardPressure.reviewLog ||= [];
  state.seasonBoardPressure.flags ||= { weeklyReview: true, objectiveWeights: true, boardPressure: true, financeGate: true, rankingGate: true };
  state.seasonBoardPressure.flags.weeklyReview ??= true;
  state.seasonBoardPressure.flags.objectiveWeights ??= true;
  state.seasonBoardPressure.flags.boardPressure ??= true;
  state.seasonBoardPressure.flags.financeGate ??= true;
  state.seasonBoardPressure.flags.rankingGate ??= true;
  ...
  state.globalScouting.score ??= 74;
  state.globalScouting.activeRegion ||= 'southAmerica';
  state.globalScouting.budgetMode ||= 'balanced';
  state.globalScouting.scoutingFocus ||= 'balanced';
  state.globalScouting.reports ||= [];
  state.globalScouting.prospects ||= [];
  state.globalScouting.watchlist ||= [];
  state.globalScouting.signedHistory ||= [];
  state.globalScouting.lastProcessedToken ??= null;
  state.globalScouting.lastAuditToken ??= null;
  state.globalScouting.auditLog ||= [];
  state.globalScouting.flags ||= { worldMap: true, hiddenPotential: true, riskModel: true, youthPipeline: true, weeklyReports: true };
  state.globalScouting.flags.worldMap ??= true;
  state.globalScouting.flags.hiddenPotential ??= true;
  state.globalScouting.flags.riskModel ??= true;
  state.globalScouting.flags.youthPipeline ??= true;
  state.globalScouting.flags.weeklyReports ??= true;
  ...
  state.playerRelationships.score ??= 76;
  state.playerRelationships.cultureMode ||= 'balanced';
  state.playerRelationships.rivalryPolicy ||= 'competitive';
  state.playerRelationships.mentorMode ||= 'auto';
  state.playerRelationships.pairings ||= [];
  state.playerRelationships.dramaLog ||= [];
  state.playerRelationships.mediationLog ||= [];
  state.playerRelationships.pressLog ||= [];
  state.playerRelationships.lastProcessedToken ??= null;
  state.playerRelationships.lastAuditToken ??= null;
  state.playerRelationships.flags ||= { rivalryModel: true, chemistryImpact: true, weeklyDrama: true, mentoring: true, pressNarrative: true };
  state.playerRelationships.flags.rivalryModel ??= true;
  state.playerRelationships.flags.chemistryImpact ??= true;
  state.playerRelationships.flags.weeklyDrama ??= true;
  state.playerRelationships.flags.mentoring ??= true;
  state.playerRelationships.flags.pressNarrative ??= true;


  state.trainingMicrocycle ||= { score: 82, loadPolicy: 'balanced', recoveryPriority: 'standard', nextWeekFocus: 'balanced', injuryPrevention: true, athleteLoadMap: {}, weeklyLog: [], auditLog: [], lastProcessedToken: null, lastAuditToken: null, flags: { periodization: true, injuryPrevention: true, facilitySynergy: true, matchEngineImpact: true, mobileLoadBoard: true } };
  state.trainingMicrocycle.score ??= 82;
  state.trainingMicrocycle.loadPolicy ||= 'balanced';
  state.trainingMicrocycle.recoveryPriority ||= 'standard';
  state.trainingMicrocycle.nextWeekFocus ||= 'balanced';
  state.trainingMicrocycle.injuryPrevention ??= true;
  state.trainingMicrocycle.athleteLoadMap ||= {};
  state.trainingMicrocycle.weeklyLog ||= [];
  state.trainingMicrocycle.auditLog ||= [];
  state.trainingMicrocycle.lastProcessedToken ??= null;
  state.trainingMicrocycle.lastAuditToken ??= null;
  state.trainingMicrocycle.flags ||= { periodization: true, injuryPrevention: true, facilitySynergy: true, matchEngineImpact: true, mobileLoadBoard: true };
  state.trainingMicrocycle.flags.periodization ??= true;
  state.trainingMicrocycle.flags.injuryPrevention ??= true;
  state.trainingMicrocycle.flags.facilitySynergy ??= true;
  state.trainingMicrocycle.flags.matchEngineImpact ??= true;
  state.trainingMicrocycle.flags.mobileLoadBoard ??= true;
  state.roster.forEach(p => {
    state.trainingMicrocycle.athleteLoadMap[p.id] ||= { acuteLoad: Math.round((p.fatigue || 20) + ((100 - (p.health || 100)) * 0.35)), chronicLoad: 42, readiness: Math.round(Math.max(25, Math.min(100, (p.health || 100) - (p.fatigue || 0) * 0.45))), injuryRisk: 12, lastFocus: 'balanced', lastDecision: 'monitorar' };
  });
